...
Меню трея автоматически переключается на запасные варианты, если основной не удалось создать.
...
2. **Альтернативный трей** — pystray с нативным меню ОС. Используется, если основной вернул ошибку. Шаг пропускается при `--no-tkinter`: основной вариант уже использует нативное меню
3. **Завершение** — если ни один вариант не запустился, в лог записывается критическая ошибка `Не удалось запустить трей (запуск): Все трей-бэкенды недоступны`. В standalone-сборке приложение завершается с кодом 1, при запуске из исходников — только предупреждение в лог
...
- При переключении на запасной вариант в логе появятся строки:
  ...
  - `Fallback на pystray (--no-tkinter) — нативное меню`
