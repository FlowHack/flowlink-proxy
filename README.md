...
>
> **Внимание:** Windows Defender иногда ложно определяет `FlowLink Proxy.exe` как `Trojan:Win32/Bearfoos.A!ml`. Это эвристическое (ML) срабатывание на PyInstaller-сборку: распаковка при запуске, использование криптографии и автозапуска. Вредоносного кода в проекте нет — исходники открыты ([github.com/FlowHack/flowlink-proxy](https://github.com/FlowHack/flowlink-proxy)). Если Defender заблокировал бэкенд, разрешите файл вручную: **«Защита от вирусов и угроз» → «Действия» → «Разрешить на устройстве»** → выберите `FlowLink Proxy.exe`. Новые сборки (onedir) уже не дают этого срабатывания. Подробнее — в разделе [«Устранение проблем»](#устранение-проблем).
