...
**Запуск браузера с расширением:** при включённом чекбоксе «Запуск с расширением» пункт 
«Запустить браузер» запускает браузер с автоматическим подключением расширения FlowLink Proxy 
(CRX-автоподкидывание через флаг `--load-extension`) — устанавливать расширение вручную не нужно.
...
**Отказоустойчивость меню:** если меню трея не удалось создать (например, отсутствует tkinter 
или произошла ошибка рендера), бэкенд автоматически переключается на альтернативный вариант — 
pystray с нативным меню ОС. Если и он недоступен, приложение завершается с записью причины 
в лог. Флаг `--no-tkinter` принудительно включает нативный вариант — подробнее в 
[DEBUG.md](DEBUG.md#--no-tkinter).
