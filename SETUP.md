...
- Автозапуск бэкенда при входе в Windows (через реестр HKCU\...\Run)
...
> **Настройка `flowlink.desktop`:** `Exec` — путь к скрипту запуска.
...
| `autostart/flowlink.desktop` | `Exec` | — | Путь к скрипту запуска |
