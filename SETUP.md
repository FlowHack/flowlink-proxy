...
### Запуск браузера с автоподкидыванием расширения

Чекбокс **«Запуск с расширением»** включает автоподкидывание расширения: при запуске браузера 
через пункт меню «Запустить браузер» расширение FlowLink Proxy загружается автоматически из 
CRX-файла (флаг `--load-extension`) — устанавливать расширение вручную не нужно.

Как это работает:

1. Отметьте чекбокс **«Запуск с расширением»** в меню трея
2. Выберите браузер через «Выбрать браузер...» (или укажите путь через `--browser-path`)
3. Нажмите «Запустить браузер» — браузер откроется с настроенным прокси и подключённым расширением

Когда использовать:

- **Быстрый старт** — расширение подключается автоматически, без ручной установки на странице расширений
- **Переустановка браузера или очистка данных** — расширение снова подхватывается при первом же запуске через меню
- **Если расширение ещё не установлено вручную** — автоподкидывание избавляет от необходимости открывать страницу расширений

CRX-файл ищется рядом с бинарником, в папке ресурсов или в директории данных. Если файл 
`flowlink-proxy.crx` не найден, браузер запустится без расширения (ошибкой это не является).

> **Важно:** При запуске браузера через бэкенд используется отдельный чистый профиль (`--user-data-dir`). Данные основного профиля браузера (закладки, пароли, расширения) в нём недоступны — это политика Chromium. Основной профиль не затрагивается.
