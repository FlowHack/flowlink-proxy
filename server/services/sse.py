...
async def handle_sse(
    ...
    # Уведомляем подключившегося клиента о готовности бэкенда. Событие
    # ставится ПОСЛЕ очистки очереди: при перезапуске бэкенда расширение
    # переподключается позже старта, поэтому ранее поставленное событие
    # было бы отброшено как устаревшее (см. очистку выше).
    await queue.put({'event': 'backend_ready', 'data': {}})
